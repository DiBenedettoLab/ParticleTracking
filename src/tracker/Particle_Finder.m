function [centroids] = Particle_Finder(inputnames,threshold,framerange,outputname,bground_name,arealim,invert,noisy)
% Usage: [centroids] = ParticleFinder(inputnames,threshold,[framerange],[outputname],[bground_name],[arealim],[invert],[noisy])
% centroids.x, centroids.y, centroids.t and centroids.[regionprops]

% TODO: input what regionprops are wanted. right now just gives a bunch I
% preselected.

% add background calculating function
...
if ~isempty(ang)
centroids.Orientation = ang(:,1);
centroids.MajorAxisLength = ang(:,2);
centroids.MinorAxisLength = ang(:,3);
centroids.Area = ang(:,4);
centroids.Perimeter = ang(:,5);
centroids.MeanIntensity = ang(:,6);
centroids.MaxIntensity = ang(:,7);
centroids.WeightedCentroid= ang(:,[8 9]);
end
