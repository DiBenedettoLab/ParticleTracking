...
if nargin<1
    error(['Usage: [vtracks,ntracks,meanlength,rmslength] = ' mfilename ...
        '(inputnames,threshold,max_disp,[bground_name],[minarea],[invert],[noisy],[framerange],[gifname],[index])'])
end
...
if ~exist('found','var') || isempty(found)
    found=found_default;
end
if ~exist('correct','var') || isempty(correct)
    correct=[];  %camara parameters to correct for lens effects
end
if ~exist('yesvels','var') || isempty(yesvels)
    yesvels=1; %calculate velocities. if zero, only makes trajectories, no vel.
end


% -=- Find particles in all frames -=-------------------------------------
if isempty(found)
[x,y,t,ang]=ParticleFinder_MHD_2022(inputnames,threshold,framerange,[], ...
...
tfound=t;
%[tt,ends]=unique(t); old
%begins=circshift(ends,[1 0])+1; old
%begins(1)=1;

% MHD & IG 05/15/2022 (issue with framerange not starting at 1 fixed)
tt2=1:max(tfound);%min(tfound):max(tfound);

[tt,endsind]=unique(tfound,'last');
[tt,beginsind]=unique(tfound,'first');

ends=ones(length(tt2),1);
begins=ends;
...
disp(['    Total number of tracks: ' num2str(numel(tracks),'%.0f')])
    ...
    if ~iscolumn(x)  %MHD 5/24/2018
        x=x';
    end
    if ~iscolumn(y)
        y=y';
    end
    if length(unique(tfound(ind)))>1
        keyboard
    end
...
% -=- Match the tracks with kinematic predictions -=----------------------
  ...
  %    velocity =(now - prior)/9; %MHD FRAME STRADDLE CORRECTION

    % and use kinematics to estimate a future position
    
%for frame straddling %MHD
%     framestraddle=0;
%     factor=9;
%     if framestraddle
%         if rem(time,2)==0
   ...
   % keyboard
    if nfr1>0
        ...
        if minarea==1
            ...
            for ii = 1:numel(unmatched)
                newtracks(ii) = struct('len',1,'X',fr1(unmatched(ii),1),...
                    'Y',fr1(unmatched(ii),2),'T',time,'Theta',ang1(unmatched(ii),:));
            end
        end
    else % if nfr1>0
        active=[];
